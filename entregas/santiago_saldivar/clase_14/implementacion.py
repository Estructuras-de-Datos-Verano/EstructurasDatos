...
def ultima_piedra(piedras: list[int]) -> int:
    ...
    """
    #Crea el heap con valores negativos para que el valor de hasta arriba sea el mayor, técnicamente.
    heap = HeapMin([-p for p in piedras])
    heap.construir_heap(heap.valores)

    #Necesitamos, al menor, dos piedras.
    while heap.tamano() > 1:

        # Sacamos las dos piedras más grandes.
        primera = -heap.extraer_min()
        segunda = -heap.extraer_min()
        # Las multiplica por menos uno para tener valores positivos.

        # Si son lo mismo, pues es cero y no le mueve nada agregar un cero.
        if primera != segunda:
            residuo = primera - segunda

            # Insertamos el residuo en negativo de vuelta al heap.
            heap.insertar(-residuo)

    # Si no hay pierdas, 0. Si hay una, esa.
    return 0 if heap.esta_vacio() else -heap.extraer_min()
