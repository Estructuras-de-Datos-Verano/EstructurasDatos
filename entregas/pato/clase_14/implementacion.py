...
class HeapMin:
    ...
    def _subir(self, indice: int) -> None:
        ...
        while indice > 0:
            ...
            if self.valores[padre] > self.valores[indice]:
...
def ultima_piedra(piedras: list[int]) -> int:
    ...
    heap_piedras = HeapMin([-p for p in piedras])

    while len(heap_piedras.valores) >= 2:
        mayor = -1 * heap_piedras.extraer_min()
        segundo = -1 * heap_piedras.extraer_min()
        if mayor != segundo:
            heap_piedras.insertar(-1 * (mayor - segundo))

    if heap_piedras.esta_vacio():
        return 0

    return -1 * heap_piedras.extraer_min()
