"""Pruebas públicas guiadas para altura, balance y degeneración en BST. Se añaden las pruebas para el estudiante"""
...
def test_todo_busqueda_balanceada():
    ...
    arbol = construir(valores_balanceados)
    valores_obtenidos = {nodo.valor for nodo in arbol.nodos_accedidos(2)}
    assert arbol.comparaciones_busqueda(2) == 3
    assert valores_obtenidos == {2,4,8}
...
def test_todo_insercion_ordenada_altura_maxima():
    ...
    for valor in range(100):
        arbol.insertar(valor)
    assert arbol.altura() == len([i for i in range(100)]) 
