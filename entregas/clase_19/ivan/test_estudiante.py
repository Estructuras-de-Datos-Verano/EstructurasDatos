...
def test_propio_varios_ordenes_validos():
    grafo = {"A": ["C"], "B": ["C"]}
...
# 2. Ciclo simple
def test_propio_ciclo_simple():
    grafo = {"A": ["B"], "B": ["A"]}
    assert orden_topologico(grafo) is None

# 3. Ciclo dentro de una parte del grafo (parcial)
def test_propio_ciclo_parcial():
    grafo = {"D": ["E"], "A": ["B"], "B": ["C"], "C": ["A"]}
    assert orden_topologico(grafo) is None

# 4. Vecino implícito
def test_propio_vecino_implicito():
    grafo = {"A": ["B"]}
    assert orden_topologico(grafo) == ["A", "B"]

# 5. Dependencias duplicadas
def test_propio_dependencias_duplicadas():
    grafo = {"A": ["B", "B"]}
    assert orden_topologico(grafo) == ["A", "B"]

# 6. Orden incorrecto
def test_propio_orden_incorrecto():
    grafo = {"A": ["B"]}
    assert not es_orden_topologico(grafo, ["B", "A"])

# 7. No mutación (Utilizando pytest.mark.parametrize)
@pytest.mark.parametrize(
    "grafo_entrada, estado_esperado",
    [
        ({"A": ["B"], "B": ["C"]}, {"A": ["B"], "B": ["C"]}),
        ({"X": ["Y", "Z"], "Y": []}, {"X": ["Y", "Z"], "Y": []}),
        ({"SoloFuentes": []}, {"SoloFuentes": []}),
    ],
)
def test_propio_no_mutacion(grafo_entrada, estado_esperado):
    orden_topologico(grafo_entrada)
    assert grafo_entrada == estado_esperado

# 8. Autoarista
def test_propio_autoarista():
    grafo = {"A": ["A"]}
    assert orden_topologico(grafo) is None

# 9. Índice válido
def test_propio_indice_valido():
    numero_cursos = 3
    prerrequisitos = [(0, 2)]  
    resultado = ordenar_cursos(numero_cursos, prerrequisitos)
    assert resultado is not None
    assert len(resultado) == numero_cursos
    assert set(resultado) == set(range(numero_cursos))
    assert resultado.index(0) < resultado.index(2)